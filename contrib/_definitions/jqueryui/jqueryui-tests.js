...
function test_droppable() {
    ...
    $("#droppable").droppable({
        ...
        drop: function (event, ui) {
            $(_this).addClass("ui-state-highlight").find("p").html("Dropped!");
        }
    });
    $("#draggable").draggable();
    $("#droppable").droppable({
...
function test_autocomplete() {
    ...
    $.widget("ui.combobox", {
        _create: function () {
            var input, that = _this, select = _this.element.hide(), selected = select.children(":selected"), value = selected.val() ? selected.text() : "", wrapper = _this.wrapper = $("<span>").addClass("ui-combobox").insertAfter(select);
    ...
    $("#birds").bind("keydown", function (event) {
        if (event.keyCode === $.ui.keyCode.TAB && $(_this).data("autocomplete").menu.active) {
    ...
    $("#city").autocomplete({
        ...
        minLength: 2,
        select: function (event, ui) {
            log(ui.item ? "Selected: " + ui.item.label : "Nothing selected, input was " + _this.value);
        },
        open: function () {
    ...
    $("#birds").autocomplete({
        ...
        minLength: 2,
        select: function (event, ui) {
            log(ui.item ? "Selected: " + ui.item.value + " aka " + ui.item.id : "Nothing selected, input was " + _this.value);
        }
    });
    $("#birds").autocomplete({
        source: data,
        minLength: 0,
        select: function (event, ui) {
            log(ui.item ? "Selected: " + ui.item.value + ", geonameId: " + ui.item.id : "Nothing selected, input was " + _this.value);
        }
    });
}
...
//@ sourceMappingURL=jqueryui-tests.js.map
